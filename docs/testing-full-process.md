...
| Packaged UI smoke | Verify packaged DM shell and preload bridge in a Playwright-controllable artifact. Hardened release binaries with `RunAsNode` disabled cannot be driven by `_electron.launch()`. | CI on `main`, scheduled/manual runs, and release candidates. | `npm run pack:qa:unfused` then `BOLTBERRY_E2E_EXECUTABLE_PATH=... npx playwright test e2e/smoke/packaged-app.spec.ts --project=smoke` |
