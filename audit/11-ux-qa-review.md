...
| **Sprint 5** — M-40 / M-44 / M-45 | ⚠ partial | M-45 LeftToolDock shipped (roving tabindex + Arrow / Home / End nav, vertical orientation, per-WAI-ARIA toolbar pattern). M-44 minimal shipped (canvas container is now tab-focusable with `role="application"` + `aria-label` so keyboard users can land on it; existing global shortcuts work from there). M-40 (keybind editor) and the deeper M-44 (per-token Tab cycling, visual focus indicator) **deferred** — both are multi-day features that need: a new `keybindStore` for overrides + conflict detection, persisted UX in GlobalSettingsModal, and integration with every keyboard handler in the app (M-40); plus a token-ordering policy, focus-visible indicator inside Konva, and interactive QA on the cycle-and-Enter flow (M-44 deeper). Tracked as their own follow-up sprint. `b8c3bac1` |
| **Backlog batch 1** — m-7 / m-15 / m-21 / m-32 / m-36 / m-43 / m-47 | ✅ shipped | `4eafaed5` — drop dead 'atmosphere' enum, delete-confirm threshold, Ctrl+Click toggle-add, menu close on resize/blur, Ctrl+D duplicate, SubToolStrip shortcut hints, focus-visible verified |
| **Backlog batch 2** — m-25 / m-26 / m-33 / m-48 | ✅ shipped | `8933ca56` — SubToolStrip arrow nav, RoomLayer Esc gate verified, hover-intent submenu (~250 ms), Toast live region |
| **Backlog deferred** — m-5 / m-6 / m-14 / m-19 / m-27 / m-31 / m-37 / m-38 / m-46 | ⚠ park | Each is a UX-decision or behavior-change-risk that wants live QA. m-5/m-6 (auto-revert tools) is opt-in UX, not a bug. m-14 (Esc ladder) changes Esc semantics across the app. m-19 (token label hit drift) is a Konva geometry edge case. m-27 (SFX HUD) needs design. m-31 (type-to-search hint) needs placement decision. m-37/m-38 are touchy a11y / multi-path features. m-46 (landmarks) is an app-wide semantic HTML pass. |
